...
#if HAVE_DECL_SCHED_SETAFFINITY && defined(CPU_ZERO) && defined(CPU_SET)
  cpu_set_t cpus;
#endif /* HAVE_DECL_SCHED_SETAFFINITY */
#if HAVE_MACH_THREAD_POLICY_H
  thread_affinity_policy_data_t affinityData;
#endif /* HAVE_MACH_THREAD_POLICY_H */
  int retcode=0;
  int finished=0;
  while (finished==0)
    {
      ...
      CPU_SET(w,&cpus);
      /* fprintf(stderr,
         "Trying to pin to cpu %d out of %d reported by the system\n",
         w,numcores); */
	...
	w=w/2;
      ...
      if(retcode) {
        fprintf(stderr,
                "MAC OS X: Failed pinning to cpu %d, trying %d/2\n",w, w);
        w=w/2;
      }
#endif /* HAVE_MACH_THREAD_POLICY_H */
      if(retcode == 0) {
        ...
        finished=1;
        w=w; /* remove warning about unused variable */
      }
    }
